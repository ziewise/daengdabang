...
export function matchesReviewedZiewcraftSingleVideo(product, reviews) {
    ...
    // The owner requires dog-worn products to complete both four-second segments.
    if (["wear", "harness", "goggles", "leash"].includes(product?.subcategory)
        || raw?.videoQuality === "approved_dog_wearing") return false;
    ...
    return Boolean(record.schema === "ddb.ziewcraft-single-hover-4s.v1" && record.publicationStatus === "approved"
        ...
        && (identity.source.catalog.isFood ? record.videoQuality === "approved_product_contents" : ["approved_dog_using", "approved_dog_interacting"].includes(record.videoQuality)) && raw.videoQuality === record.videoQuality
