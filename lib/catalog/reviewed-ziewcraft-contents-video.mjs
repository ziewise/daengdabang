...
export const sameContentsBinding = (a, b) => binding(a) && binding(b) && a.artifact_id === b.artifact_id && a.sha256 === b.sha256;
export const CONTENTS_REVIEW_CHECKS = ["contentsVisible", "packageAndFlavorMatch", "contentsShapeTextureMatch",
    "contentsQuantityAndScaleMatch", "noInventedContents", "naturalPhysicalMotion", "fixedComposition",
    "noProductDeformation", "noProhibitedOverlays", "fullClipWatched", "loopWatched"];
export const CONTENTS_SERVER_CHECKS = ["product_identity", "anatomy_and_motion", "camera_and_framing",
...
export function contentsCatalogSnapshot(raw) {
...
function sourceValid(source) {
    ...
    return object(snapshot) && same(Object.keys(snapshot).sort(), [...SOURCE_FIELDS].sort())
        ...
        && text(snapshot.name) && text(snapshot.brandEn) && snapshot.isFood === true && imagePath(snapshot.image)
        && (snapshot.sourceUrl === null || imagePath(snapshot.sourceUrl))
        ...
        && source.sceneContentsVerified === true;
}
export function validContentsTechnical(validation) {
...
function terminalValid(terminal, segment) {
    ...
    return binding(terminal) && sameContentsBinding(terminal, p)
...
/** A shop single-clip policy, not strict 8-second delivery or a server publication grant. */
export function validZiewcraftContentsIdentity(identity) {
    if (!object(identity) || identity.kind !== "ziewcraft_human_review_contents_4s.v1" || identity.provider !== "ziewcraft"
    ...
    if (!object(s) || !id(s.jobId) || !hash(s.ownerSha256) || s.status !== "succeeded" || s.delivery_mode !== "human_review"
        ...
        || !terminalValid(s.review_terminal, s) || !validContentsTechnical(s.validation)
    ...
    if (!object(h) || h.actor !== "human" || !text(h.evidenceId) || h.evidenceId.length > 200 || !hash(h.sha256)
        ...
        || !CONTENTS_REVIEW_CHECKS.every(key => h.checks?.[key] === true)
    ...
    if (!object(r) || r.api_job_id !== s.jobId || !id(r.review_id) || r.status !== "approved"
        ...
        || !text(r.reviewer) || !text(r.note) || !CONTENTS_SERVER_CHECKS.every(key => r.checks?.[key] === true)
        || r.checks.pair_and_loop_watched != null || r.bindings?.pair_video != null
        || !sameContentsBinding(r.bindings?.native_video, s.native_video) || !sameContentsBinding(r.bindings?.review_video, v)
        || !sameContentsBinding(r.bindings?.terminal, s.review_terminal)
...
export function matchesReviewedZiewcraftContentsVideo(product, reviews) {
    ...
    if (!validZiewcraftContentsIdentity(identity) || !same(identity, record?.videoZiewcraftIdentity)) return false;
    const s = identity.segment, review = record.review, technical = record.technical;
    return Boolean(record.schema === "ddb.ziewcraft-contents-hover-4s.v1" && record.publicationStatus === "approved"
        && record.productId === product.id && product.id === `p_${raw.no}` && record.folder === folder && raw.folder === folder
        && raw.supplierCatalogHistorical !== true && same(contentsCatalogSnapshot(raw), identity.source.catalog)
        ...
        && review.scope === "actual_product_contents_single_4_second_clip"
        && CONTENTS_REVIEW_CHECKS.every(key => review.checks?.[key] === true)
        && review.checks.fullDecode === true && review.checks.branding === true
        && hash(review.loopReportSha256) && Array.isArray(review.limitations) && review.limitations.every(text));
}
