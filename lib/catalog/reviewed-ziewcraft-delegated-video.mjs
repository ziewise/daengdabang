...
// These catalog entries are replacement lenses, not complete dog-worn goggles.
const replacementLensIds = new Set(['p_74', 'p_75', 'p_76', 'p_95', 'p_98', 'p_416', 'p_417']);
...
export function matchesReviewedZiewcraftDelegatedVideo(product, records) {
    ...
    const handDemonstration = record.videoQuality === 'approved_product_interaction';
    const lensDemonstration = handDemonstration && replacementLensIds.has(product.id);
    const worn = ['wear', 'harness'].includes(product.subcategory)
        || (product.subcategory === 'goggles' && !lensDemonstration)
        || (product.subcategory === 'leash' && !handDemonstration)
        || record.videoQuality === 'approved_dog_wearing';
    // A hand demonstrating an accessory is distinct from footage of a dog using it.
    if (handDemonstration && (seconds !== 4 || worn || raw.isFood)) return false;
