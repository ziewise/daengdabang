...
- 우리 푸터의 `FOOTER_LEGAL_LINKS` 가 `#terms`·`#privacy` 앵커(placeholder)였음.
- 협업자가 만든 실제 페이지로 연결: `#terms → /terms`, `#privacy → /privacy`.
- 환불·반품/분쟁해결은 협업자 페이지 미존재 → 앵커 유지.

### 7-B-2. 푸터 사업자 정보 — 단일 출처화 (`components/footer/Footer.tsx`)
- **발견**: 협업자가 `lib/legal.ts` 의 `BUSINESS_INFO`(환경변수 기반)에 회사 정보를
  이미 정의해 둠 (상호 주식회사 포엔치, 대표 박주연, 사업자번호 873-81-03438,
  통신판매업 제2026-성남수정-0227호, 주소, 고객센터 1800-2162, 호스팅 GitHub Pages).
- 우리 푸터는 placeholder(홍길동/000-00-00000 등)를 **하드코딩**하고 있었음.
...
우리 디자인 토큰(aurora·glass·크레파스 배경)에 맞춰 작성.
...
> **1:1 문의**: 운영(협업자)에 문의 폼·메일 발송 백엔드가 없어, 클릭 시 메일 앱이 열리는
...
> **협업자 파일(app/api, lib/store, lib/daengdabang-llm, lib/customer-api, lib/legal,
