...
test("the full storefront catalog keeps the reviewed dog-pack boundary", async () => {
    ...
    assert.deepEqual(rows.filter((row) => detected.includes(`p_${row.no}`))
        .map((row) => row.supplierGoodsNo).sort(), [
        "1000000230", "1000000984", "1000000985", "1000001165", "1000001182", "1000001311",
    ]);
});
...
test("catalog re-review preserves the withheld set alongside separately approved content edits", async () => {
    ...
    assert.deepEqual(Object.keys(overrides).sort(), [...new Set([...expected.allOverrideFolders,...Object.keys(newlyApproved),...motionWithdrawnFolders])].sort());
