...
test("mobile header PetLens reveals the hero lens cue before opening on the next tap", async () => {
    ...
    assert.match(watermark, /shouldStageMobileCueBeforeOpen/);
    assert.match(watermark, /window\.matchMedia\("\(max-width: 1023px\)"\)\.matches/);
...
test("result prioritizes urgent guidance and otherwise leads with target attribution and translation", async () => {
    ...
    assert.match(result, /data-daenglab-overall-summary/);
    assert.match(result, /펫렌즈 전체 총평/);
    assert.match(result, /분야별 핵심 관찰/);
    assert.match(result, /data-daenglab-inference-empty-state=\{config\.kind\}/);
    assert.match(result, /표시할 신호가 없어 차트는 생략했어요/);
    assert.match(result, /data-daenglab-inference-static-summary=\{config\.kind\}/);
    assert.match(result, /시간 좌표가 없는 결과라 임의의 빈 차트를 만들지 않고, 아래 후보 요약만 보여드려요/);
    assert.doesNotMatch(result, /min-h-44 place-items-center/);
    ...
    assert.match(result, /\{highPriorityUrgency && <UrgencyGuidance result=\{result\} highPriority \/>\}[\s\S]*<OverallObservationSummary result=\{result\} \/>[\s\S]*<TargetAttributionPanel result=\{result\} \/>/);
