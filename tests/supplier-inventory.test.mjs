...
function loadModule(path, mocks = {}, append = '') {
    ...
    vm.runInNewContext(compiled+append,{module:loadedModule,exports:loadedModule.exports,console,require:(id)=>{
        ...
        if(id.includes('catalog/live-inventory'))return {useLiveInventoryProduct:p=>p,liveInventoryProduct:p=>p};
...
test('generated contract covers only public metadata and resolver mirrors every current option',()=>{
    ...
    for(const [folder,entry] of Object.entries(document.products)) {
        ...
        if(currentFolders.has(folder))assert.match(entry.sourceDate,/^\d{4}-\d{2}-\d{2}$/,folder);
    ...
    // Live observations may change availability, but cannot add or lose an
    // exact selectable combination or change the supplier fulfillment policy.
    assert.equal(observed.current.available,observed.current.supplier_request);
    assert.equal(observed.current.available+observed.current.sold_out+observed.current.unknown,1920);
