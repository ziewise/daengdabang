...
assert.match(html, /\/videos\/goods-contest-hero-mobile\.mp4/);
assert.match(html, /<video[^>]*autoPlay=""[^>]*muted=""[^>]*loop=""[^>]*playsInline=""[^>]*preload="auto"/);
assert.match(html, /<source[^>]*media="\(max-width: 640px\)"[^>]*goods-contest-hero-mobile\.mp4/);
assert.match(html, /object-contain/);
assert.doesNotMatch(html, /18초 굿즈 미리보기|영상 소리 켜기|영상 음소거/);
