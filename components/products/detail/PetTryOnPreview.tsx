...
export default function PetTryOnPreview({
    ...
    useEffect(() => {
        let active = true;
        setLocalPetPreview(null);
        if (!petReferenceImage) return () => { active = false; };
        void prepareImageOnDevice(petReferenceImage).then((prepared) => {
            if (active) setLocalPetPreview(prepared);
        });
        return () => { active = false; };
    }, [petReferenceImage]);
