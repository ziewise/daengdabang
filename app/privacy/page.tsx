...
const noConsentRows = [
    ...
    [
        "가입 혜택 휴대전화 인증",
        "정규화된 휴대전화번호 HMAC 별칭, 요청 IP HMAC, OTP HMAC, 인증 요청·만료·시도·상태 기록. 휴대전화번호 원문과 OTP 원문은 서버 DB에 저장하지 않으며 휴대전화번호 원문은 발송 시 선택된 문자 사업자에 일시 전달",
        "가입 혜택 인증번호 발송·검증, 재전송·시도 제한 및 오남용 방지",
        "인증번호 만료 후 24시간",
    ],
    [
        "가입 혜택 중복 방지",
        "정규화된 인증 휴대전화번호를 서버 비밀키로 HMAC 변환한 일방향 식별값, 혜택 프로그램·키 버전, 식별값 생성일시",
        "신규 가입 댕랩코인 20C의 인증 휴대전화번호 1개당 1회 지급, 탈퇴·재가입 반복 수령 및 부정 이용 방지",
...
const outsourcingRows = [
    ...
    ["가입 혜택 문자 인증", "선택된 문자 발송 사업자(연동 시 사업자명 고지 및 정책 반영)", "인증번호 발송을 위해 휴대전화번호 원문을 발송 처리 중 일시 전달"],
