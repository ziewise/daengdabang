...
const noConsentRows = [
    [
        "회원 가입·관리",
        "이름, 생년월일, 회원ID, 비밀번호, 이메일주소, 연계정보(CI)",
    ...
    [
        "결제·환불",
        "회원ID, 주문내역, 신용카드 결제시(카드사명, 승인·거래 식별정보), 계좌이체/환불시(계좌정보, 입금자이름), 휴대폰결제시(휴대전화번호, 통신사명), 간편결제서비스 이용시(이름, 휴대전화번호, 이메일주소), 현금영수증 신청시(휴대전화번호, 현금영수증 카드번호), 승인·취소 기록. 카드번호·유효기간·CVC는 PG사 보안창에서 처리되며 댕다방이 직접 수집하거나 저장하지 않음",
        "대금 결제, 취소, 환불, 정산, 부정거래 확인",
        "대금결제 및 재화 공급 기록 5년",
    ],
    [
        "고객·사업 문의",
        "이름, 회사·브랜드·기관명, 휴대전화번호, 이메일주소, 홈페이지 주소(입점 제안 시), 주문번호, 상품·수량·예산·희망일·배송지역, 상담·제안 내용, 첨부자료",
        "고객 문의 응대, 불만 처리, 입점·제휴 및 대량 구매 검토, 분쟁 대응",
    ...
    [
        "가입 혜택 이메일 인증",
        "정규화된 이메일 HMAC 별칭, 요청 IP HMAC, OTP HMAC, 인증 요청·만료·시도·상태 기록. 이메일주소는 회원 계정 정보로 보관되고 발송 시 이메일 발송 사업자에 전달되며, OTP 원문은 서버 DB에 저장하지 않음",
        "가입 혜택 인증번호 발송·검증, 재전송·시도 제한 및 오남용 방지",
        "인증번호 만료 후 24시간",
    ],
    [
        "가입 혜택 중복 방지",
        "정규화된 인증 이메일을 서버 비밀키로 HMAC 변환한 일방향 식별값, 혜택 프로그램·키 버전, 식별값 생성일시",
        "신규 가입 댕다방 연구소 코인 20C의 중복 수령 탐지, 반복 가입 등 부정 이용 방지 및 부정 지급 혜택 회수",
...
const consentRows = [
    ...
    [
        "성장 프로그램 관심등록",
        "회원 식별번호, 선택 프로그램, 동의 버전·상태·시각. 실제 안내 시 회원 계정 이메일을 연결해 사용하며 관심등록 파일에는 이메일을 복사 저장하지 않음",
        "프로그램 수요 확인 및 준비·모집 소식 이메일 안내",
        "관심등록 취소 시 즉시 삭제 또는 회원 탈퇴 시까지",
    ],
    [
        "이벤트·리뷰",
...
const outsourcingRows = [
    ...
    ["행동·소리 자동 분석", "Google LLC", "이용자가 명시 동의한 댕다방 연구소 행동·소리 관찰 결과 생성"],
];
...
const overseasRows = [
    [
        ...
        "이용자가 댕다방 연구소 행동·소리 분석에 명시적으로 동의하고 분석을 실행할 때 암호화된 HTTPS 통신으로 이전",
        "영상·음성에서 관찰되는 신호와 가능한 맥락을 분석하여 결과 생성",
        "분석 기능 운영을 위한 임시 처리는 최대 24시간, 안전·오남용 방지 기록은 최대 55일 범위에서 제공자 정책에 따라 제한적으로 처리될 수 있음",
        "동의하지 않거나 전송을 거부할 수 있으며, 이 경우 댕다방 연구소 행동·소리 분석 기능을 이용할 수 없음",
    ],
];
    ...
    return (
        <main className="mx-auto max-w-[1040px] px-4 py-10 md:px-6">
            ...
            <section id="overseas" className="mt-8 scroll-mt-24 border-t border-neutral-200 pt-5">
                ...
                <p className="mt-2 text-sm font-bold leading-7 text-neutral-600">
                    댕다방 연구소 행동·소리 분석은 이용자가 별도로 동의하고 직접 실행할 때 분석에 필요한 정보만 보안 연결로
            ...
            <section id="automation" className="mt-8 scroll-mt-24 border-t border-neutral-200 pt-5">
                ...
                <p className="mt-2 text-sm font-bold leading-7 text-neutral-600">
                    ...
                    재분석을 요청할 수 있습니다. 반려견 사진은 댕다방이 관리하는 로컬 분석 환경에서 처리하며, 로컬 분석에
                    문제가 생겨도 Google·OpenAI 등 외부 사진 분석 API로 자동 전송하지 않습니다. 댕다방 연구소 행동·소리
                    분석은 별도 동의 후 짧은 영상과 포함 음성을 자동으로 살펴 참고 결과를 제공합니다. 댕다방이 자체 학습한 수의학적 진단 모델이나 수의사의 진단을 대신하는 기능이 아닙니다.
                </p>
            </section>
